...
# New tags
$tagPub = @{ Key="Name"; Value="Public - Added by Jimmy - PowerShell" }
$tagPvt = @{ Key="Name"; Value="Private - Added by Jimmy - PowerShell" }

# Tag specifications
# Public subnet
$tagSpecPub = new-object Amazon.EC2.Model.TagSpecification
$tagSpecPub.ResourceType = "subnet"
$tagSpecPub.Tags.Add($tagPub)
# Private subnet
$tagSpecPvt = new-object Amazon.EC2.Model.TagSpecification
$tagSpecPvt.ResourceType = "subnet"
$tagSpecPvt.Tags.Add($tagPvt)

# Create two new subnets in the third AZ
$privateSubnet = New-EC2Subnet -AvailabilityZone $az.ZoneName `
     -CidrBlock "10.0.12.0/24" -VpcId $vpc.VpcId -TagSpecification $tagSpecPub
$publicSubnet = New-EC2Subnet -AvailabilityZone $az.ZoneName `
     -CidrBlock "10.0.2.0/24" -VpcId $vpc.VpcId -TagSpecification $tagSpecPvt
...
# New resources from Jimmy
Write-Output ($JimmysResources.GetEnumerator() | 
Sort -Property Name)
